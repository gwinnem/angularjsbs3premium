 ...
 * @author Alex_Crack
 * Bugfixed by Geirr Winnem - ABAdmin author. 
 * @version v0.3.6
 * @link https://github.com/alexcrack/angular-ui-notification
 * @license MIT
 * @summary Angular.js service providing simple notifications using Bootstrap 3 styles with css transitions for animating
 */
...
angular.module('ui-notification').provider("$notification", function () {
    ...
    this.$get = ["$timeout", "$http", "$compile", "$templateCache", "$rootScope", "$injector", "$sce", "$q", "$window",
        ...
        notify.primary = function (message, title, delay) {
            var args = {
                message: message, title: title, delay: delay
            }
            return this(args, 'primary');
        };
        notify.error = function (message, title, delay) {
            var args = {
                message: message, title: title, delay: delay
            }
